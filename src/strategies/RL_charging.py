...
class RLChargingStrategy(SchedulingStrategy):
    """时间优先 + 拼单 + RL 充电时机策略。"""
    ...
    def __init__(self) -> None:
        ...
        # RL 充电决策（轻量在线Q学习，仅用于是否充电）
        self.rl_learning_rate = 0.15
        self.rl_epsilon = 0.08
        self.rl_hard_charge_ratio = 0.12
        self.rl_q_table: Dict[Tuple[int, int, int, int], List[float]] = {}
        self._rl_rng = random.Random(20260414)
    ...
    def _apply_idle_rebalance(
        ...
        if pending_task_count > 0 or urgent_task_count > 0:
            for vehicle_id in idle_ids:
                vehicle = context.vehicles[vehicle_id]
                default_should_charge = self._battery_ratio(vehicle) < context.config.low_battery_ratio
                charge_plan = self._build_charge_plan_with_rl(
                    vehicle=vehicle,
                    context=context,
                    pending_task_count=pending_task_count,
                    urgent_task_count=urgent_task_count,
                    default_should_charge=default_should_charge,
                )
                if charge_plan is not None:
                    plans[vehicle_id] = charge_plan
            return
        ...
        for vehicle_id in idle_ids:
            ...
            if vehicle_id in keep_ids:
                default_should_charge = self._battery_ratio(vehicle) < self.depot_charge_ratio_no_pressure
                charge_plan = self._build_charge_plan_with_rl(
                    vehicle=vehicle,
                    context=context,
                    pending_task_count=pending_task_count,
            ...
            charge_plan = self._build_charge_plan_with_rl(
                ...
                pending_task_count=pending_task_count,
                urgent_task_count=urgent_task_count,
                default_should_charge=True,
            )
            if charge_plan is not None:
                plans[vehicle_id] = charge_plan

    def _build_charge_plan_with_rl(
        self,
        vehicle: Vehicle,
        context: StrategyContext,
        pending_task_count: int,
        ...
        should_charge = self._rl_should_charge(
            vehicle=vehicle,
            context=context,
            pending_task_count=pending_task_count,
            urgent_task_count=urgent_task_count,
    ...
    def _rl_should_charge(
        ...
        battery_ratio = self._battery_ratio(vehicle)
        if battery_ratio <= self.rl_hard_charge_ratio:
            return True
        ...
        q_values = self.rl_q_table.setdefault(obs, [0.0, 0.0])

        if self._rl_rng.random() < self.rl_epsilon:
            action = self._rl_rng.choice([self._ACTION_KEEP, self._ACTION_CHARGE])
        else:
            action = self._ACTION_CHARGE if q_values[self._ACTION_CHARGE] > q_values[self._ACTION_KEEP] else self._ACTION_KEEP

        reward = self._rl_proxy_reward(
        ...
        q_values[action] += self.rl_learning_rate * (reward - q_values[action])

        return action == self._ACTION_CHARGE
    ...
    def _rl_observation(
        ...
        battery_ratio = self._battery_ratio(vehicle)
        battery_bin = min(4, int(battery_ratio * 5.0))

        backlog_ratio = pending_task_count / max(1, len(context.vehicles))
        backlog_bin = min(3, int(backlog_ratio * 2.0))

        urgent_bin = 1 if urgent_task_count > 0 else 0

        pressure = max(0.0, station.pressure_index())
        pressure_bin = min(3, int(min(3.99, pressure)))
        return battery_bin, backlog_bin, urgent_bin, pressure_bin

    def _rl_proxy_reward(
        self,
        battery_ratio: float,
        pending_task_count: int,
        urgent_task_count: int,
        ...
        pressure_penalty = min(1.0, max(0.0, station.pressure_index()) / 4.0)
        task_pressure = 1.0 if (pending_task_count > 0 or urgent_task_count > 0) else 0.0

        if action == self._ACTION_CHARGE:
            reward = (1.0 - battery_ratio) * 1.1
            reward -= pressure_penalty * 0.35
            reward -= task_pressure * 0.20
        else:
            reward = battery_ratio * 0.75
            reward += task_pressure * 0.35
            if battery_ratio < self.rl_hard_charge_ratio + 0.08:
                reward -= 0.90

        if default_should_charge and action == self._ACTION_CHARGE:
            reward += 0.12
        if default_should_charge and action == self._ACTION_KEEP:
            reward -= 0.12
